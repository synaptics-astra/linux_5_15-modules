						...
						rtk_uint32 data)
{
	...
	ret = rtl83xx_smi_write(phy_id, register_id, (unsigned short) data);

	/* if (ret)
		printk("%s: got error code %d from miiphy_write\n", __func__, ret); */
}
...
void MDC_MDIO_READ(	rtk_uint32 preamble_len,
					...
					rtk_uint32 *pData)
{
	*pData = (rtk_uint32)rtl83xx_smi_read(phy_id, register_id);
}
